...
export class ProductsService {
  ...
  private async clearProductsCache(): Promise<void> {
    const cache = this.cacheManager as any; // Просимо TypeScript не втручатися
    
    if (typeof cache.reset === 'function') {
      await cache.reset(); // Для старих версій бібліотеки
    } else if (typeof cache.clear === 'function') {
      await cache.clear(); // Для нових версій бібліотеки
    }
  }
  ...
  async create(dto: CreateProductDto): Promise<Product> {
    ...
    await this.clearProductsCache(); // <-- Додано виклик методу
    return saved;
  }
